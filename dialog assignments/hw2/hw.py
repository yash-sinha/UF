...
print "Speaker 1 identifier is -> "+ sp2_identifier
...
cap_w_person1 = [word.upper() for word in w_person1] #capitalizes all the words
cap_w_person2 = [word.upper() for word in w_person2] #capitalizes all the words

w_person1_counts = Counter(cap_w_person1) #counts the number each time a word appears
w_person2_counts = Counter(cap_w_person2) #counts the number each time a word appears

common_person1= [ite for ite, it in Counter(w_person1_counts).most_common(1)]
common_person2= [ite for ite, it in Counter(w_person2_counts).most_common(1)]
...
common_bigrams_corpus = bigram_corpus_dist.keys()

trigram_corpus=nltk.trigrams(w_total)
trigram_corpus_dist = nltk.FreqDist(trigram_corpus)
common_trigrams_corpus = trigram_corpus_dist.keys()
...
print "Most common bigram in corpus "+str(common_bigrams_corpus[:1])
print "Most common trigram in corpus "+str(common_trigrams_corpus[:1])
...
print "Mode of words spoken by Speaker 1 is "+str(median_person1)
print "Mode of words spoken by Speaker 2 is "+str(median_person2)
